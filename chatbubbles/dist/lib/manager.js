...
import { configureBubbles, hookBubbles, isNativeAvailable, unhookBubbles, } from './bubbles';
...
function updateBubbleAppearance() {
    ...
    configureBubbles(Math.round(Number(avatarRadius) || 0), Math.round(Number(bubbleChatRadius) || 0), getBubbleColor()).catch(e => {
        console.error('[ChatBubbles] bubbles.configure failed:', e);
    });
}
export function startBubbles(cleanup) {
    let stop = () => { };
    void (async () => {
